...
class PortfolioOptimizer:
    ...
    def generate_efficient_frontier(self, returns: pd.DataFrame, n_portfolios: int = 100,
        ...
        min_vol_weights = self.optimize_min_volatility(returns, constraints)
        min_vol_return = np.sum(returns.mean() * min_vol_weights) * 252
        
        max_return = returns.mean().max() * 252 * 0.9  # 90% of max individual return
        ...
        for target_return in target_returns:
            try:
                weights = self.optimize_target_return(returns, target_return, constraints)
    ...
    def optimize_portfolio(self, symbols: List[str], method: str = 'max_sharpe',
        ...
        if method == 'max_sharpe':
            optimal_weights = self.optimize_max_sharpe(returns, constraints)
